...
function cleanAndParseJSON(content: string): any {
  ...
  throw new Error(
    `Failed to parse JSON response after trying 4 strategies. Response length: ${content.length}`
  );
}
...
export const felipeController = {
  ...
  startDecisionMaking: async (c: Context) => {
    ...
    console.log(
      "📝 Generating specific Polymarket keywords for this decision..."
    );
    ...
    try {
      const keywordsJson = cleanAndParseJSON(
        polymarketKeywordsResponse.content
      );
      specificKeywords = keywordsJson.keywords || [];
      console.log(
        `✅ Successfully parsed ${specificKeywords.length} specific keywords:`,
        specificKeywords
      );
    } catch (error) {
    ...
    if (specificKeywords.length > 0) {
      ...
      try {
        const markets = await polymarketService.searchMultipleKeywords(
          specificKeywords
        );
        specificMarkets = polymarketService.filterByRelevance(markets, 100);
        // console.log(
        //   `✅ Found ${specificMarkets.length} decision-specific markets`
        // );
      } catch (error) {
    ...
    console.log(
      `📊 Combined market pool: ${trendingMarkets.length} trending + ${specificMarkets.length} specific = ${allMarkets.length} total unique markets`
    );

    console.log("ALL MARKETS:");
    console.log(allMarkets.map((m) => `- ${m.question}`).join("\n"));

    // Step 2: Build enriched context from Polymarket
...
Los siguientes son ${
        allMarkets.length
      } mercados relevantes de Polymarket con probabilidades basadas en dinero real.
...
- Si ningún mercado es relevante para una consecuencia, deja "relatedMarketIds" vacío`
    ...
    console.log(
      `📚 Context built: ${allMarkets.length} Polymarket markets (${trendingMarkets.length} trending + ${specificMarkets.length} specific)`
    );
    ...
    const systemPrompt = `
      ...
      - Incluye también consecuencias de baja probabilidad (1-10%) si son relevantes
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(
          `🎲 Attempt ${attempt}/${maxRetries} to generate consequences`
        );
        ...
        console.log(
          `✅ Successfully parsed ${consequences.length} ${
            inputType === "question" ? "scenarios" : "consequences"
          } on attempt ${attempt}`
        );
    ...
    consequences = consequences.map((consequence: any) => {
      ...
      if (
        consequence.relatedMarketIds &&
        Array.isArray(consequence.relatedMarketIds)
      ) {
        ...
        for (const marketId of consequence.relatedMarketIds) {
          ...
          if (market) {
            ...
            console.log(
              `  ✅ Matched market "${market.question}" to consequence "${consequence.nombre}"`
            );
          } else {
            console.log(
              `  ⚠️  Market ID "${marketId}" not found in available markets`
            );
          }
        }
      }
      ...
      if (relatedMarkets.length > 0) {
        const avgPolymarketProbability = Math.round(
          relatedMarkets.reduce((sum, market) => sum + market.probability, 0) /
            relatedMarkets.length
        );
        console.log(
          `  🔄 Overriding LLM probability ${consequence.probabilidad}% with Polymarket average ${avgPolymarketProbability}% for "${consequence.nombre}"`
        );
    ...
    console.log(
      "✅ Generated",
      consequences.length,
      "consequences with market mappings"
    );
  ...
  getDecisions: async (c: Context) => {
    ...
    try {
      const decisions = await decisionFirestoreService.getDecisionsByUserId(
        email
      );
  ...
  getDecisionById: async (c: Context) => {
    ...
    try {
      const decision = await decisionFirestoreService.getDecisionById(
        decisionId
      );
  ...
  expandConsequence: async (c: Context) => {
    const body = await c.req.json();
    const { decisionId, nodeId, originalDecision, consequence, email } = body;

    if (!consequence || !email) {
      return c.json({ error: "Consequence and email are required" }, 400);
    }

    if (!decisionId || !nodeId) {
      return c.json(
        { error: "DecisionId and nodeId are required for persistence" },
        400
      );
    }
    ...
    console.log(
      `📌 Original decision/question: "${originalDecision || "N/A"}"`
    );
    ...
    // Step 1: Get trending Polymarket markets
    console.log("🔥 Fetching trending Polymarket markets...");
    let trendingMarkets: PolymarketMarket[] = [];
    try {
      trendingMarkets = await polymarketService.getTrendingMarkets(25);
    ...
    // Step 2: Generate specific Polymarket keywords for this consequence expansion
    console.log(
      "📝 Generating specific Polymarket keywords for consequence expansion..."
    );
    const polymarketKeywordsPrompt = `
      Analiza esta consecuencia y el contexto original, y genera 5-8 keywords/phrases EN INGLÉS muy específicos para buscar mercados de predicción relevantes en Polymarket.

      CONTEXTO ORIGINAL:
      ${originalDecision || "N/A"}

      CONSECUENCIA QUE ESTAMOS EXPANDIENDO:
      - Nombre: ${consequence.nombre}
      - Descripción: ${consequence.descripcion}

      IMPORTANTE:
      - Las keywords DEBEN ser en INGLÉS
      - Deben ser específicas a eventos futuros que podrían ocurrir SI esta consecuencia se materializa
      - Piensa en mercados "macro" relevantes: economía global, política mundial, tecnología, etc.

      Ejemplos:
      - Consecuencia: "Mi startup fracasa" → ["AI bubble burst", "tech recession 2025", "startup funding winter", "tech layoffs"]
      - Consecuencia: "Bitcoin cae a 20k" → ["crypto crash", "recession 2025", "federal reserve rates", "stock market crash"]
      - Consecuencia: "Guerra comercial con China" → ["US China war", "global recession", "supply chain crisis", "world war 3"]
    ...
    // Step 3: Search for specific markets related to this consequence
    let specificMarkets: PolymarketMarket[] = [];
    if (specificKeywords.length > 0) {
      console.log("🔍 Searching for consequence-specific markets...");
      try {
        const markets = await polymarketService.searchMultipleKeywords(
          specificKeywords
        );
        specificMarkets = polymarketService.filterByRelevance(markets, 100);
        console.log(
          `✅ Found ${specificMarkets.length} consequence-specific markets`
        );
      } catch (error) {
        console.error("⚠️ Error searching specific markets:", error);
      }
    ...
    // Step 4: Combine trending + specific markets (remove duplicates)
    const allMarkets: PolymarketMarket[] = [];
    const seenIds = new Set<string>();
    ...
    // Step 5: Use LLM to intelligently filter ONLY specific markets
    // Trending markets are always kept (they're macro/general events)
    let filteredSpecificMarkets = specificMarkets;
    if (
      consequence.relatedMarkets &&
      Array.isArray(consequence.relatedMarkets) &&
      consequence.relatedMarkets.length > 0 &&
      specificMarkets.length > 0
    ) {
      console.log(`🤖 Using LLM to filter SPECIFIC markets intelligently...`);
      console.log(
        `📌 Trending markets (${trendingMarkets.length}) will be kept without filtering`
      );

      // Build context about parent markets
      const parentMarketsContext = consequence.relatedMarkets
        .map((m: PolymarketMarket) => `- "${m.question}" (ID: ${m.id})`)
        .join("\n");

      // Build context about ONLY specific markets (not trending)
      const specificMarketsContext = specificMarkets
        .map((m) => `- [ID: ${m.id}] "${m.question}"`)
        .join("\n");

      const filterPrompt = `
Eres un asistente experto en análisis de mercados de predicción.

CONTEXTO:
El usuario está expandiendo una consecuencia que tiene estos mercados de Polymarket asociados:
${parentMarketsContext}

TAREA:
De la siguiente lista de mercados ESPECÍFICOS, debes identificar cuáles NO deberían usarse porque son del MISMO EVENTO o periodo temporal que los mercados del padre.

IMPORTANTE: Estos son solo mercados específicos relacionados a la decisión. Los mercados trending/macro ya están incluidos por defecto.

Por ejemplo:
- Si el padre tiene "Will Kast win Chilean election?", NO incluyas "Will Boric win?", "Will Matthei win?", o cualquier otro candidato de la MISMA elección
- Si el padre tiene "Bitcoin price 2025", NO incluyas "Bitcoin 100k 2025" o mercados del mismo año/evento
- SÍ incluye mercados sobre CONSECUENCIAS FUTURAS o eventos diferentes

MERCADOS ESPECÍFICOS DISPONIBLES:
${specificMarketsContext}

Devuelve SOLO los IDs de los mercados que SÍ deberían estar disponibles (excluyendo los del mismo evento que el padre).

Responde ÚNICAMENTE con un JSON válido:
{
  "allowedMarketIds": ["id1", "id2", ...]
}
`;

      try {
        const filterResponse = await llmServiceManager.generateText(
          {
            prompt: filterPrompt,
        ...
        console.log("📥 Received filter response, attempting to parse...");
        const filterJson = cleanAndParseJSON(filterResponse.content);
        const allowedIds = new Set(filterJson.allowedMarketIds || []);

        console.log(
          `✅ LLM allowed ${allowedIds.size} specific markets out of ${specificMarkets.length}`
        );

        // Filter ONLY specific markets based on LLM decision
        const beforeCount = specificMarkets.length;
        filteredSpecificMarkets = specificMarkets.filter((market) =>
          allowedIds.has(market.id)
        );

        const filteredCount = beforeCount - filteredSpecificMarkets.length;
        console.log(
          `🚫 LLM filtered out ${filteredCount} specific markets: ${beforeCount} → ${filteredSpecificMarkets.length} specific markets`
        );

        if (filteredCount > 0 && filteredCount <= 10) {
          // Show which markets were filtered (only if not too many)
          const filtered = specificMarkets.filter((m) => !allowedIds.has(m.id));
          console.log(
            `   Specific markets removed by LLM:`,
            filtered
              .slice(0, 5)
              .map((m) => `"${m.question}"`)
              .join(", ")
        ...
        console.error(
          "⚠️ Error filtering specific markets with LLM, keeping all specific markets:",
          error
        );
        // On error, keep all specific markets (fail safe)
        filteredSpecificMarkets = specificMarkets;
      }
    } else {
      console.log(
        `📝 No filtering needed (no parent markets or no specific markets)`
      );
    }

    // Step 5b: Combine trending (unfiltered) + specific (filtered)
    const filteredMarkets: PolymarketMarket[] = [];
    const seenFilteredIds = new Set<string>();

    // Add all trending markets (NEVER filtered)
    for (const market of trendingMarkets) {
      if (!seenFilteredIds.has(market.id)) {
        filteredMarkets.push(market);
        seenFilteredIds.add(market.id);
      }
    }

    // Add filtered specific markets
    for (const market of filteredSpecificMarkets) {
      if (!seenFilteredIds.has(market.id)) {
        filteredMarkets.push(market);
        seenFilteredIds.add(market.id);
      }
    ...
    console.log(
      `📊 Final market pool: ${trendingMarkets.length} trending (unfiltered) + ${filteredSpecificMarkets.length} specific (filtered) = ${filteredMarkets.length} total markets`
    );

    // Log final markets for debugging
    console.log(
      `\n📋 Final markets that will be available to LLM (${filteredMarkets.length} total):`
    );
    filteredMarkets.forEach((market, idx) => {
      const source = trendingMarkets.some((m) => m.id === market.id)
        ? "[TRENDING]"
        : "[SPECIFIC]";
      console.log(
        `   ${idx + 1}. ${source} "${market.question}" (${
          market.probability
        }%, ends: ${market.endDate || "N/A"})`
      );
    });
    console.log("");

    // Step 6: Build Polymarket context for LLM
    let polymarketContext = "";
    if (filteredMarkets.length > 0) {
      let parentMarketsNote = "";
      if (consequence.relatedMarkets && consequence.relatedMarkets.length > 0) {
        parentMarketsNote = `\n\nNOTA: Los mercados trending/macro están SIEMPRE disponibles (AI, economía, política global, etc.). Solo los mercados específicos del mismo evento que la consecuencia padre han sido excluidos.`;
      }
...
Incluye tanto mercados populares como mercados específicos relacionados a esta consecuencia.
Para cada consecuencia secundaria que generes, debes seleccionar 0-5 mercados relevantes de esta lista usando sus IDs.${parentMarketsNote}
...
IMPORTANTE sobre cómo usar Polymarket:
- Para cada consecuencia secundaria, DEBES seleccionar mercados relevantes de la lista anterior
- Usa el ID del mercado (ej: "0x1234...") en el campo "relatedMarketIds"
- Solo selecciona mercados que sean REALMENTE relevantes para esa consecuencia específica
- Las probabilidades de Polymarket deben INFLUIR en tus estimaciones de probabilidad
- Si ningún mercado es relevante para una consecuencia, deja "relatedMarketIds" vacío`;
    } else {
      // No markets available (should rarely happen since trending are always included)
      polymarketContext = `

📊 MERCADOS DE POLYMARKET:
No hay mercados disponibles en este momento.
Para todas las consecuencias secundarias, usa "relatedMarketIds": [] (vacío).`;
    }

    console.log(
      `📚 Polymarket context built with ${filteredMarkets.length} markets`
    );

    // Build context with both original decision and current consequence
    const contextualPrompt = originalDecision
      ? `CONTEXTO ORIGINAL:
Tu decisión/pregunta inicial fue: "${originalDecision}"

ESCENARIO ACTUAL:
Estamos explorando el futuro donde la siguiente consecuencia ocurre:
- Nombre: "${consequence.nombre}"
- Descripción: ${consequence.descripcion}
- Probabilidad de que ocurra: ${consequence.probabilidad}%

TAREA:
Ahora analiza: ¿Qué 10 nuevas consecuencias o futuros podrían derivarse de que "${consequence.nombre}" ocurra en el contexto de "${originalDecision}"?`
      : `La consecuencia "${consequence.nombre}" va a ocurrir.

Descripción: ${consequence.descripcion}

Ahora analiza: ¿Qué 10 nuevas consecuencias podrían derivarse de que esto ocurra?`;

    const expansionPrompt = contextualPrompt;

    const systemPrompt = `
Eres Felipe, un asistente experto en análisis de consecuencias y exploración de futuros posibles.

El usuario te ha dado:
1. Una decisión/pregunta ORIGINAL (el punto de partida)
2. Una consecuencia específica que ya fue simulada previamente (un escenario futuro)

Ahora debes explorar QUÉ PASARÍA SI ESA CONSECUENCIA OCURRE, manteniendo siempre en mente el contexto de la decisión/pregunta original.

IMPORTANTE: Las nuevas consecuencias deben ser coherentes con AMBOS contextos:
- El contexto original (la decisión/pregunta inicial del usuario)
- El escenario actual (la consecuencia que estamos expandiendo)

Genera exactamente 10 posibles consecuencias secundarias que podrían derivarse de que la consecuencia primaria ocurra en el contexto de la decisión original.
...
`;
    ...
    try {
      ...
      const parsedResponse = cleanAndParseJSON(llmResponse.content);
      let expandedConsequences = parsedResponse.consequences || [];
      console.log(
        `✅ Generated ${expandedConsequences.length} expanded consequences`
      );

      // Map market IDs to actual market objects
      console.log("🔗 Mapping market IDs to market objects...");
      ...
      expandedConsequences = expandedConsequences.map((cons: any) => {
        const relatedMarkets: PolymarketMarket[] = [];

        if (cons.relatedMarketIds && Array.isArray(cons.relatedMarketIds)) {
          // Look up each market ID
          for (const marketId of cons.relatedMarketIds) {
            const market = marketMap.get(marketId);
            if (market) {
              relatedMarkets.push(market);
              console.log(
                `  ✅ Matched market "${market.question}" to consequence "${cons.nombre}"`
              );
            } else {
              console.log(
                `  ⚠️  Market ID "${marketId}" not found in available markets`
        ...
        // Override LLM probability with Polymarket average if markets exist
        let finalProbability = cons.probabilidad;
        if (relatedMarkets.length > 0) {
          const avgPolymarketProbability = Math.round(
            relatedMarkets.reduce(
              (sum, market) => sum + market.probability,
              0
            ) / relatedMarkets.length
          );
          console.log(
            `  🔄 Overriding LLM probability ${cons.probabilidad}% with Polymarket average ${avgPolymarketProbability}% for "${cons.nombre}"`
          );
          finalProbability = avgPolymarketProbability;
        }

        // Remove relatedMarketIds from the final response (we don't need to send IDs to frontend)
        const { relatedMarketIds, ...consWithoutIds } = cons;
      ...
      console.log(
        "✅ Mapped",
        expandedConsequences.length,
        "consequences with market data"
      );

      const consequencesWithMarkets = expandedConsequences;

      // Save expanded consequences to Firestore
      console.log(`💾 Saving expanded consequences to Firestore...`);
      try {
        await decisionFirestoreService.updateNestedConsequence(
          decisionId,
          email,
          nodeId,
          consequencesWithMarkets
        );
        console.log(`✅ Successfully saved expanded consequences to Firestore`);
      } catch (firestoreError) {
        console.error(
          "⚠️ Error saving to Firestore (non-fatal):",
          firestoreError
        );
        // Continue even if Firestore save fails - don't break user experience
      }
